...
nsgif::nsgif(FILE *file, size_t size)
{
    ...
    gif_pointer = &gif_struct;

    /* create our gif animation */
	memset(gif_pointer, 0, sizeof(gif_animation));
	gif_pointer->decoded_frame = GIF_INVALID_FRAME;

    gif_data = (unsigned char *)malloc(size);
    if (!gif_data)
    {
    ...
    size_t n = fread(gif_data, 1, size, file);
    if (n != size)
    {
        return;
    }
    ...
    do
    {
        code = gif_initialise(gif_pointer, n, gif_data);
        if (code != GIF_OK && code != GIF_WORKING)
        {
            printf("gif_initialise %i",code);
            return;
        }
    }
    while (code != GIF_OK);
...
unsigned char* nsgif::get_image(){
    return (unsigned char *) gif_pointer->frame_image;
}

int nsgif::get_width(){
    return gif_pointer->width;
}

int nsgif::get_height(){
    return gif_pointer->height;
}

int nsgif::get_time(unsigned int frame){
    return gif_pointer->frames[frame].frame_delay*10;
}

int nsgif::get_count(){
    return gif_pointer->frame_count;
}

int nsgif::get_current_frame() {
	return gif_pointer->decoded_frame;
}

int nsgif::get_gif_result() {
	return gif_pointer->current_error;
}

unsigned int nsgif::gif_interlaced_line(int height, int y) {
...
bool nsgif::gif_next_LZW(gif_animation *gif) {
	int code, incode;
	int block_size;
	int new_code;
 ...
 */
void nsgif::gif_init_LZW(gif_animation *gif) {
	int i;
...
int nsgif::gif_next_code(gif_animation *gif, int code_size) {
	...
	if (!clear_image) {
		...
		*/
		if ((frame == 0) || (gif->decoded_frame == GIF_INVALID_FRAME)) {
			memset((unsigned char*)frame_data, GIF_TRANSPARENT_COLOUR, width * height * sizeof(int));
...
/**	Skips the frame's extensions (which have been previously initialised)
	...
	@return GIF_INSUFFICIENT_FRAME_DATA for insufficient data to complete the frame
		GIF_OK for successful decoding
*/
nsgif::gif_result nsgif::gif_skip_frame_extensions(gif_animation *gif) {
	unsigned char *gif_data, *gif_end;
	int gif_bytes;
...
*/
nsgif::gif_result nsgif::gif_initialise(gif_animation *gif, size_t size, unsigned char *data) {
...
*/		gif_data += 3;
		...
		*/
		if ((gif->frame_image = gif_bitmap_cb_create(gif->width, gif->height)) == NULL) {
			gif_finalise(gif);
			return GIF_INSUFFICIENT_MEMORY;
		}
...
/**	Attempts to initialise the next frame
	...
	@return GIF_INSUFFICIENT_DATA for insufficient data to do anything
		...
		GIF_WORKING for successful decoding if more frames are expected
*/
nsgif::gif_result nsgif::gif_initialise_frame(gif_animation *gif) {
	int frame;
	gif_frame *temp_buf;
...
*/
nsgif::gif_result nsgif::gif_initialise_frame_extensions(gif_animation *gif, const int frame) {
	unsigned char *gif_data, *gif_end;
	int gif_bytes;
	unsigned int block_size;
...
/**	Updates the sprite memory size
	...
	@return GIF_INSUFFICIENT_MEMORY for a memory error
		GIF_OK for success
*/
nsgif::gif_result nsgif::gif_initialise_sprite(gif_animation *gif, unsigned int width, unsigned int height) {
	...
	*/
	if ((buffer = (bitmap*)gif_bitmap_cb_create(max_width, max_height)) == NULL)
		return GIF_INSUFFICIENT_MEMORY;
	gif_bitmap_cb_destroy(gif->frame_image);
...
*/
void nsgif::gif_finalise(gif_animation *gif) {
...
void *nsgif::gif_bitmap_cb_create(int width, int height)
{
    return malloc(width * height*sizeof(int));
}


void nsgif::gif_bitmap_cb_set_opaque(void *bitmap, bool opaque)
{
    (void) opaque;  /* unused */
    assert(bitmap);
}
...
bool nsgif::gif_bitmap_cb_test_opaque(void *bitmap)
{
    assert(bitmap);
    return false;
}
...
unsigned char *nsgif::gif_bitmap_cb_get_buffer(void *bitmap)
{
    assert(bitmap);
...
void nsgif::gif_bitmap_cb_destroy(void *bitmap)
{
    assert(bitmap);
    free(bitmap);
}
...
void nsgif::gif_bitmap_cb_modified(void *bitmap)
{
    assert(bitmap);
    return;
}
